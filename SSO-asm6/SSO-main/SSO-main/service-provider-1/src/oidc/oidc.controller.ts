...
export class OidcController {
  ...
  @Get('login')
  @Render('login')
  getLoginPage(@Res() res: Response, @Req() req: Request) {
    // If this Get is redirect from SP then save SAML resquest in session

    const redirect_link =
      this.IdP_oidc_loginpage +
      '?response_type=code&client_id=sp1&redirect_uri=http://127.0.0.1:3001/oidc/callback&scope=openid&state=random_state_value';
  ...
  async processAuthorization(
    ...
    try {
      ...
      const des_id_token = await this.jwtService.verifyAsync(
