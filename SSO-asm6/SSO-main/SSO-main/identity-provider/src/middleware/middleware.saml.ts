  ...
  async use(@Req() req: Request, @Res() res: Response, next: NextFunction) {
    ...
    if (req.session.samlreq) {
      ...
      await this.samlService
        .decodeAndParseSamlRequest(req.session.samlreq)
        .then((jsonResponse) => {
      ...
      const decode_res_result = this.samlService.generateSamlResponse(
        requestId,
        ascServiceURL,
      ...
      const redirect_link =
        ascServiceURL + '?SAMLResponse=' + decode_res_result;
