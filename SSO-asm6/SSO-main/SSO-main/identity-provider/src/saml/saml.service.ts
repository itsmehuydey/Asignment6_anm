...
export class SamlService {
  ...
  public authenticate(user_name: string, pass_word: string) {
    console.log(
      '---------- Input username - password: ' + user_name + ' - ' + pass_word,
      '\n',
    );
    const result = false;
    for (const i in login_data) {
      if (
        user_name == login_data[i].username &&
        pass_word == login_data[i].password
      ) {
        return true;
      }
    }
  ...
  public decodeAndParseSamlRequest(encodedRequest: string): Promise<any> {
    try {
      ...
      // Log decoded request
      console.log(
        '---------- Decoded SAMLAuthnRequest sent from SP: \n',
        samlRequest,
      );
  ...
  public generateSamlResponse(
    ...
    const samlResponse = `<samlp:AuthnRequest xmlns="urn:oasis:names:tc:SAML:2.0:protocol" ID="5678" InResponseTo="${requestId}" Version="2.0" IssueInstant="2024-03-24T12:00:03Z" Destination="${assertionConsumerServiceURL}">
    				<Issuer>http://127.0.0.1:3000</Issuer>
				<samlp:Status>
        				<samlp:StatusCode Value="urn:oasis:names:tc:SAML:2.0:status:Success"/>
    				</samlp:Status>
				<saml:Assertion ID="assertion5678" Version="2.0" IssueInstant="2024-11-24T12:00:03Z" xmlns="urn:oasis:names:tc:SAML:2.0:assertion">
        				<saml:Issuer>http://127.0.0.1:3000</saml:Issuer>
        				<saml:Subject>
            					<saml:NameID>${user}</saml:NameID>
        				</saml:Subject>
        				<saml:Conditions NotBefore="2024-11-25T08:00:00Z"
                         				NotOnOrAfter="2024-11-25T08:05:00Z"/>
        				<saml:AuthnStatement AuthnInstant="2024-11-25T08:00:00Z">
            					<saml:AuthnContext>
